...
TARGET_ARMS = ("uniform", "linear_noaudio", "linear_donor", "sigmoid_donor", "sharpening")
...
def build_teacher_target(
    ...
    *,
    ...
    if arm == "uniform":
        ...
        if arm != "sharpening" and (negative_logits is None or negative_logits.shape != real_logits.shape):
    ...
    for begin in range(0, real_flat.shape[0], position_chunk_size):
        ...
        if arm == "uniform":
            ...
            ratio = torch.zeros_like(log_real)
            log_target = F.log_softmax(real * (1.0 + strength), dim=-1)
        else:
    ...
    statistics.update(
        ...
        target_tau=(1.0 / (1.0 + strength) if arm == "sharpening" else float(tau)),
...
class AudioTargetOPDTrainer(KeOPDV10UniformOPDTrainerV1):
    ...
    def _opd_loss(self, model: torch.nn.Module, row: dict[str, Any], real_waveform: Any):
        ...
        if arm not in ("uniform", "sharpening"):
        ...
        metrics = dict(
            ...
            teacher_forward_calls_observed=1 + int(arm not in ("uniform", "sharpening")),
            teacher_negative_forward_count=int(arm not in ("uniform", "sharpening")),
            ...
            negative_same_completion=float(arm not in ("uniform", "sharpening")),
            **target_statistics,
        )
